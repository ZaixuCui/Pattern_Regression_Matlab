...
function SVR_W_Permutation_4(Data_Path, Scores, Perm_times_Range, ResultantFolder, Queue)

for i = Perm_times_Range
    i
    Rand_Index = randperm(length(Scores));
    Rand_Scores(i, :) = Scores(Rand_Index);
end
for i =Perm_times_Range
    i
    ...
    pipeline.(Job_Name).opt.para2 = Rand_Scores(i, :);
